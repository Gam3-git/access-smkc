...
import {convertTimeAppoint} from "../services/convert_text.service";

const ViewAppoint = (props) =>  {
  ...
  const handleSearch = (data) => {
    // const state = { id: data.target.id, typesearch: data.target.getAttribute('typesearch') };
    // navigate("/casesearch", { state, target: "_blank" });
  ...
  const columns = [
    ...
    { name: 'รายละเอียด',  cell: (row) => 
    <>
      - {row.appoint} <br /> เวลา : {convertTimeAppoint(row.t_appoint)} บัลลังก์ 
      : {row.room} <br /> ข้อหา : {row.plaint} 
    </>, grow: 2, 
      },
    { name: '_', cell: (row) => 
      <>
  ...
  return (
    ...
    <div className="card-header bg-success"> 
      <p className="text-light text-center"> พบข้อมูล ในวันนัดพิจารณา ทั้งหมด : { casedata && <>{ casedata.length } คดี</> } </p> 
    </div>

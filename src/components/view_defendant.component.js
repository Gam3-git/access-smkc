...
const ViewDefendant = (props) =>  {
  ...
  const columns = [
    ...
    { name: 'คู่ความ', cell: (row) => 
      <>
        โจทก์ : {row.plaintiff} <br /> คู่ความ : {row.defendant} ({row.type_def})
      </>, grow: 2,
      },
  ...
  useEffect(() => {
    ...
    let data_plaint = casedata.filter( item => item.plaint ? item.plaint.includes(filterText) : ""  );
      // let data = data_defendant.length > 0 ? data_defendant : (data_plaintiff.length > 0 ? data_plaintiff : data_plaint);
